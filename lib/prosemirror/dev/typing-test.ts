...
export interface TypingTestConfig {
  ...
  /** Test mode */
  mode: 'standard' | 'burst' | 'concurrent' | 'chunk';
  /** Chunk size for chunk mode (default: 1000 chars) */
  chunkSize?: number;
}
...
export async function runTypingTest(
  ...
  // In burst/chunk mode, sample frames less frequently to reduce measurement overhead
  const frameSampleRate = cfg.mode === 'burst' ? 10 : isChunkMode ? 50 : 1;
  ...
  // Only press Enter for non-chunk mode
  // Chunk mode will handle document structure differently
  if (!isChunkMode) {
    pressEnter(view);
  }

  // CHUNK MODE: Insert entire elements as complete nodes for maximum speed
  if (isChunkMode) {
    for (let elemIdx = 0; elemIdx < elements.length; elemIdx++) {
      ...
      try {
        // Create a complete node with the correct element type and text
        const nodeType = screenplaySchema.nodes[element.type];
        if (!nodeType) {
          console.error('[ChunkMode] Unknown element type:', element.type);
          errors.push(`Unknown element type: ${element.type}`);
          continue;
        }

        // Debug: Log first 5 elements to verify types are correct
        if (elemIdx < 5 && process.env.NODE_ENV === 'development') {
          console.log('[ChunkMode]', {
            idx: elemIdx,
            elementType: element.type,
            nodeTypeName: nodeType.name,
            textPreview: element.text?.slice(0, 30),
          });
        }

        const { state } = view;
        const textContent = element.text ? screenplaySchema.text(element.text) : undefined;
        const node = nodeType.create({}, textContent);

        // Insert at end of document content
        const insertPos = state.doc.content.size;
        let tr = state.tr.insert(insertPos, node);

        // Move cursor to end of inserted content and scroll into view
        ...
        metrics.keystrokeLatencies.push(txTime);
        charsSoFar += element.text.length;
      } catch (e) {
        errors.push(`Failed to insert element ${element.type}: ${e}`);
      }

      // Sample frame time occasionally
      if (elemIdx % frameSampleRate === 0) {
        const frameTime = await measureFrameTime();
        metrics.frameTimes.push(frameTime);
      }

      // Track memory every 10 elements
      if (elemIdx % 10 === 0) {
        const currentMem = getMemoryMB();
        metrics.memorySnapshots.push(currentMem);
        if (currentMem > memoryPeak) memoryPeak = currentMem;
      }

      // Report progress every 10 elements
      if (onProgress && elemIdx % 10 === 0) {
      ...
      // Yield to UI briefly every 50 elements
      if (elemIdx % 50 === 0) {
        await sleep(0);
      }
    }
  } else {
    // STANDARD/BURST MODE: Character by character
    for (let elemIdx = 0; elemIdx < elements.length; elemIdx++) {
      if (cfg.abortSignal?.aborted) {
        errors.push('Test aborted by user');
        break;
  ...
  // Chunk mode inserts entire elements - very high dropped frames expected (60% ok)
  // Burst mode is fast typing - 20% dropped ok
  // Standard tests require <5% dropped frames
  const droppedThreshold = cfg.mode === 'chunk' ? 0.60 : cfg.mode === 'burst' ? 0.20 : 0.05;
  const latencyThreshold = cfg.mode === 'chunk' ? 50 : cfg.mode === 'burst' ? 30 : 50;
