...
export function parseScreenplayMarkdown(content: string): TestElement[] {
  ...
  while (i < lines.length) {
    ...
    // Inline character + dialogue: "SAMAEL What if - you know?"
    // Handles malformed lines where character name and dialogue are on same line
    const inlineCharMatch = line.match(/^([A-Z][A-Z']+)\s+(.+)$/);
    if (inlineCharMatch) {
      const [, charName, dialogueText] = inlineCharMatch;
      // Verify it's a plausible character name (2-20 chars, not a keyword)
      const excludedWords = ['THE', 'CUT', 'INT', 'EXT', 'FADE', 'CLOSE', 'WIDE', 'THEN', 'SOUND', 'AERIAL', 'TRACKING', 'EXTREME', 'BLACK', 'ANGLE', 'SHOT', 'BACK', 'SUPER', 'TITLE', 'CARD'];
      if (charName.length >= 2 && charName.length <= 20 && !excludedWords.includes(charName)) {
...
// Cache for loaded screenplays
let lyraCache: TestElement[] | null = null;
let azraelCache: TestElement[] | null = null;

/**
 * Load and parse a screenplay from the private folder.
 * Files must be in private/ folder (gitignored, dev only).
 */
async function loadPrivateScreenplay(filename: string): Promise<TestElement[]> {
  try {
    // In dev mode, we can read from the filesystem via API
    const response = await fetch(`/api/dev/screenplay?file=${encodeURIComponent(filename)}`);
    if (!response.ok) {
      throw new Error(`Failed to load ${filename}: ${response.status}`);
    }
    const content = await response.text();
    return parseScreenplayMarkdown(content);
  } catch (error) {
    console.error(`[loadPrivateScreenplay] Error loading ${filename}:`, error);
...
/**
 * Load and parse LYRA.md screenplay from private folder.
...
export async function loadLyraScreenplay(): Promise<TestElement[]> {
  ...
  lyraCache = await loadPrivateScreenplay('LYRA.md');
  return lyraCache;
}

/**
 * Load and parse AZRAEL.md screenplay from private folder.
 * Caches the result for subsequent calls.
 */
export async function loadAzraelScreenplay(): Promise<TestElement[]> {
  if (azraelCache) {
    return azraelCache;
  }
  azraelCache = await loadPrivateScreenplay('AZRAEL.md');
  return azraelCache;
}
