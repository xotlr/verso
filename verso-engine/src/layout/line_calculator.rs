...
impl<'a> LineCalculator<'a> {
    ...
    fn wrap_text(&self, text: &str, chars_per_line: usize) -> Vec<String> {
        ...
        for paragraph in text.split('\n') {
            ...
            for word in words {
                // Use char count for Unicode safety (not byte length)
    ...
    /// Uses character indices to handle UTF-8 safely (prevents panic on multi-byte chars)
    fn break_long_word(&self, word: &str, chars_per_line: usize) -> Vec<String> {
        let mut lines = Vec::new();
        let mut remaining = word;

        while remaining.chars().count() > chars_per_line {
            // Find byte position at character boundary to avoid UTF-8 panic
