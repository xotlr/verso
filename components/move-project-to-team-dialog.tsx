...
interface MoveProjectToTeamDialogProps {
  open: boolean;
...
export function MoveProjectToTeamDialog({
  open,
  onOpenChange,
  projectId,
  projectName,
  ...
  const loadTeams = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Filter out the current team if project is already in one
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const data = await response.json();
        toast.error(data.error || 'Failed to move project');
      }
    } catch (error) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Choose a team for &ldquo;{projectName}&rdquo;
          </DialogDescription>
        </DialogHeader>
